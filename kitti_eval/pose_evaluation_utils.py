...
def my_dump_pose_seq_TUM(out_file, poses, times):
    # First frame as the origin
    first_pose = pose_vec_to_mat(poses[0])
    with open(out_file, 'w') as f:
        for p in range(len(times)):
            ...
            # this_pose = np.dot(this_pose, np.linalg.inv(first_pose))
            this_pose = np.dot(first_pose, np.linalg.inv(this_pose))
            f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12,_,_,_,_= np.reshape(this_pose,16)
            f.write('R%s: %f %f %f %f %f %f %f %f %f %f %f %f\n' % (times[p],f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f11,f12))