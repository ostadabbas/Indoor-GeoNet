...
class GeoNetModel(object):
    ...
    def __init__(self, opt, tgt_image, src_image_stack, depth_seq, intrinsics):
        self.opt = opt
        self.tgt_image = self.preprocess_image(tgt_image)
        self.src_image_stack = self.preprocess_image(src_image_stack)
        self.depth_seq = tf.cast(depth_seq, dtype=tf.float32)/1000
    ...
    def build_model(self):
        ...
        # depth images concatenated along batch dimension
        if self.depth_seq != None:
            self.depth_concat = tf.concat([self.depth_seq[:,:,:,i:(i+1)] \
                                    for i in range(opt.seq_length)], axis=0)
            self.depth_concat_pyramid = self.scale_pyramid(self.depth_concat, opt.num_scales)
    ...
    def build_losses(self):
        ...
        for s in range(opt.num_scales):
            ...
            if opt.depth_weight > 0:
                ...
                groundtruth_depth = tf.concat([self.depth_concat_pyramid[s][bs*(opt.seq_length//2):bs*(opt.seq_length//2+1),:,:],
                                    self.depth_concat_pyramid[s][:bs*(opt.seq_length//2),:,:],
                                    self.depth_concat_pyramid[s][bs*(opt.seq_length//2+1):,:,:]]
                                    , axis=0)
                
                scale_1 = tf.stack([self.get_mean(groundtruth_depth[i]) for i in range(bs*opt.seq_length)])
                scale_2 = tf.stack([self.get_mean(self.pred_depth[s][i]) for i in range(bs*opt.seq_length)])
                scale = tf.tile(tf.reshape(scale_2/scale_1, [-1,1,1,1]), [1,opt.img_height//(2**s),opt.img_width//(2**s),1])
                
                zero = tf.constant(0, dtype=tf.float32)
                where = tf.not_equal(groundtruth_depth, zero)
            
                depth_loss += opt.depth_weight* opt.seq_length * tf.pow(tf.losses.mean_squared_error(
                                                 tf.boolean_mask(groundtruth_depth, where)*tf.boolean_mask(scale, where),
                                                 tf.boolean_mask(self.pred_depth[s], where)),0.5)
        ...
        if opt.mode == 'train_rigid':
            self.total_loss += rigid_warp_loss + disp_smooth_loss + depth_loss
        if opt.mode == 'train_flow':
            self.total_loss += flow_warp_loss + flow_smooth_loss + flow_consistency_loss + depth_loss
    ...
    def get_mean(self, v):
        v = tf.reshape(v, [-1])
        zero = tf.constant(0, dtype=tf.float32)
        where = tf.not_equal(v, zero)
        v = tf.boolean_mask(v, where)
        #v = tf.Session.run(v)
        m = v.get_shape()[0]//2
        return tf.reduce_mean(v) #tf.nn.top_k(v, m).values[m-1]
