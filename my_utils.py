...
def pixel2cam(depth, pixel_coords, intrinsics, is_homogeneous=True):
  ...
  cam_coords = np.matmul(inv(intrinsics), pixel_coords) * depth
  if is_homogeneous:
...
def cam2pixel(cam_coords, proj):
  ...
  cam_coords = np.reshape(cam_coords, [batch, 4, -1])
  unnormalized_pixel_coords = np.matmul(proj, cam_coords)
  x_u = unnormalized_pixel_coords[:, 0, :]
  x_u = x_u.reshape((x_u.shape[0], 1, x_u.shape[1]))
  y_u = unnormalized_pixel_coords[:, 1, :]
  y_u = y_u.reshape((y_u.shape[0], 1, y_u.shape[1]))
  z_u = unnormalized_pixel_coords[:, 2, :]
  z_u = z_u.reshape((z_u.shape[0], 1, z_u.shape[1]))
  x_n = x_u / (z_u + 1e-10)
  y_n = y_u / (z_u + 1e-10)
  pixel_coords = np.concatenate([x_n, y_n], axis=1)
  pixel_coords = np.reshape(pixel_coords, [batch, 2, height, width])
...
def meshgrid(batch, height, width, is_homogeneous=True):
  ...
  x_t = np.matmul(np.ones((height, 1)),
                  np.transpose(np.expand_dims(
                      np.linspace(-1.0, 1.0, width), 1), [1, 0]))
  y_t = np.matmul(np.expand_dims(np.linspace(-1.0, 1.0, height), 1),
                  np.ones((1, width)))
  ...
  coords = np.tile(np.expand_dims(coords, 0), [batch, 1, 1, 1])
  return coords
...
def compute_rigid_flow(depth, pose, intrinsics, reverse_pose=False):
  ...
  return rigid_flow, src_pixel_coords, tgt_pixel_coords

def my_compute_rigid_flow(depth, pose, intrinsics, reverse_pose=False):
  """Compute the rigid flow from target image plane to source image
  ...
  # Construct pixel grid coordinates
  pixel_coords = meshgrid(batch, height, width)
  tgt_pixel_coords = np.transpose(pixel_coords[:,:2,:,:], [0, 2, 3, 1])
  # Convert pixel coordinates to the camera frame
  cam_coords = pixel2cam(depth, pixel_coords, intrinsics)
  # Construct a 4x4 intrinsic matrix
  filler = np.array([0.0, 0.0, 0.0, 1.0])
  filler = filler.reshape((1, 1, 4))
  filler = np.tile(filler, [batch, 1, 1])
  intrinsics = np.concatenate([intrinsics, np.zeros([batch, 3, 1])], axis=2)
